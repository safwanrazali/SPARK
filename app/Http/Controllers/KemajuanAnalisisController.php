 ...
 *   Peringkat 5        "Jana Laporan" → Pegawai Analisis
 *                      "Hantar"       → Pegawai Analisis (kepada PPA)
 *   Peringkat 6        "Hantar"       → PPA (kepada KB)
...
class KemajuanAnalisisController extends Controller
{
    ...
    public function selesai(Request $request, string $agencyCode, int $stage)
    {
        ...
        // Peringkat "Analisis Data" hanya boleh ditutup setelah dapatan
        // inventori disimpan sebagai Lengkap — draf sahaja tidak memadai.
        if ($stage === WorkflowStatus::STAGE_ANALISIS && ! $this->analisisLengkap($agencyCode)) {
            return back()->withErrors([
                'stage' => 'Simpan Dapatan bagi Analisis Inventori Kriptografi perlu Lengkap sebelum peringkat ini boleh ditandakan Selesai.',
            ]);
        }
    ...
    /**
     * "Jana Laporan" — peringkat 5 bermula.
     *
     * Statusnya menjadi Dalam Proses, BUKAN Selesai: laporan hanya selesai
     * setelah Ketua Bahagian mengesahkannya (carta aliran bahagian 7).
     */
    public function janaLaporan(Request $request, string $agencyCode)
    {
        Gate::authorize('advance-analysis-stage');
        ...
        if (! $this->analisisLengkap($agencyCode)) {
            return back()->withErrors([
                'stage' => 'Dapatan analisis perlu Lengkap sebelum laporan boleh dijana.',
            ]);
        }
        ...
        $this->semakan->mulakan($entiti);

        return back()->with('success', sprintf(
            'Laporan bagi %s telah dijana. Semak dengan "Betulkan", kemudian tekan "Hantar".',
            $entiti['agency_code'],
        ));
    }

    /**
     * "Hantar" oleh Pegawai Analisis — laporan diserahkan kepada PPA.
     */
